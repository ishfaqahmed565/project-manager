@props(['header','prevInfo'=> null,'projectId'=> null])
        ...
        <div class="grid max-w-xl grid-cols-2 gap-4 m-auto">
            <div class="col-span-2 lg:col-span-1"> 
                <div class=" relative">
                        ...
                        placeholder="Title" value="{{old('title',$prevInfo?->title)}}"   />
                        @error('title')
                ...
                <div class="col-span-2 lg:col-span-1">
                    <div class=" relative ">
                        ...
                        <option value="active" {{ old('status', $prevInfo?->status) === 'active'? 'selected':''}}>Active</option>
                        <option value="finished" {{ old('status', $prevInfo?->status) === 'finished'? 'selected':''}}>Finished</option>
                        ...
                        <textarea class="flex-1 appearance-none
                         border border-gray-300 w-full py-2 px-4
                          ...
                          id="description" placeholder="Enter your comment"
                           name="description" rows="5" cols="40">{{old('description',$prevInfo?->description)}}</textarea>
                        @error('description')
                            <p class='text-red-500 text-xm mt-2'>{{$message}}</p>
                        @endError
                    </div>
                    <div class="col-span-2 text-right">
                        <div class="flex flex-row {{$prevInfo? 'justify-between' : 'justify-end' }} ">
                            <button  class="py-2 px-4  bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500 focus:ring-offset-indigo-200 text-white  transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-lg ">
                                Send
                            </button>
                            @if($prevInfo)
                            <div>
